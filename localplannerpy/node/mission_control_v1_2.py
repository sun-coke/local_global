...
class Subgoal_scan():
    ...
    def check_path_across(self,left_part,right_part):
        ...
        if direction == -1:# left
            sec_pos,ang = self.pol2cart(self.front_obstacle[4])
            end_ = self.arr_to_pose2D(sec_pos,ang)
            end  = self.list_H[-1][0]
    ...
    def turnpoint_g(self,end=Pose2D(),end_= Pose2D()):
        ...
        turnpoint.orientation.z = numpy.sin(theta/2)
        turnpoint.orientation.w = numpy.cos(theta/2)
        # todo fix
        return turnpoint
