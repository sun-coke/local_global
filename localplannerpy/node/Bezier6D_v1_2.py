...
class Bezier():
    def __init__(self,odom=Odometry(),goal=Pose()):
        self.numSteps = 100
        self.ctrlpx = []
        self.ctrlpy = []
        self.state = []
    ...
    def point(self, odom=Pose(), goal=Pose()):
        ...
        P = range(n)
        d1 = max(0.1,v*2.0)
        ## define by the initial velocity
        d2 = v*3.0
        d5 = vg*2.0
        # define by the demanding velocity of the subgoal
    ...
    def xy_U(self,x, y):
        ...
        w_line = []
        h = 0.1
        # the meaning of step h ralation to time  h = 0.1s = '10Hz publish rate '
        for i in range(n - 2):
            state = Robot_state()
